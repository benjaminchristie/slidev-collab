<!--
  Status — a small inline pill for the state of a work item. The same component
  the weekly theme ships, for the roadmap and future-work slides of a talk.

    <Status done />  <Status wip />  <Status blocked />  <Status ask />
    <Status wip label="under review" />
-->
<template>
  <span class="collab-status" :class="`is-${kind}`">{{ text }}</span>
</template>
...
.is-done    { background: rgba(46, 139, 87, 0.13);  color: var(--color-green-deep); }
.is-wip     { background: rgba(42, 143, 189, 0.13); color: var(--color-blue); }
.is-blocked { background: rgba(190, 60, 55, 0.12);  color: var(--color-red); }
.is-ask     { background: rgba(204, 112, 0, 0.14);  color: var(--color-emphasis); }
</style>
